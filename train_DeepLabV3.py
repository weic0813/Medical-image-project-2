...
from evaluate_DeepLabV3 import evaluate
from unet.unet_model import UNet
from utils.data_loading import BasicDataset, CarvanaDataset
from utils.dice_score import dice_loss


dir_img = Path('./data/OD/Drishti-GS/train/imgs/')
dir_mask = Path('./data/OD/Drishti-GS/train/masks/')
...
def train_model(
    ...
    #criterion = nn.CrossEntropyLoss() if model.classifier.num_classes > 1 else nn.BCEWithLogitsLoss()
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    mask = torch.argmax(masks_pred, dim=1).cpu().numpy()
                    color = [(0, 0, 0), (255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 255)]
                    color_matrix = np.zeros((mask.shape[1], mask.shape[2], 3), dtype=np.uint8)
                    for i in range(mask.shape[1]):
                        for j in range(mask.shape[2]):
                            color_matrix[i, j] = color[mask[0][i, j]]
                    cv2.imwrite('./test.jpg', color_matrix)
                    
                    # if model.classifier.num_classes == 1:
                    ...
                    loss += dice_loss(
                        F.softmax(masks_pred, dim=1).float(),
                        F.one_hot(true_masks, 5).permute(0, 3, 1, 2).float(),
                        multiclass=True
                    )
